...
def usage():
  """
    Shows usage and exits with return code 1
  """
  print(f"""
    Copies the html code passed on the command line to the clipboard.
    usage: {os.path.basename(sys.argv[0])}  'html-code'
    Example:
      {os.path.basename(sys.argv[0])} '<a href="http://sonar-band.ch">Sonar</a>'
...
if __name__ == '__main__':
    ...
    PutHtml(data)
    if GetHtml() == data:
        print(f"Clipboard now contains HTML passed on command line ({data})")
    else:
        print("Failed.")
